...
**Observer.** Same backend and isolation, Opus 5, effort `high`, `output_format` against §6. `ember observe <session_dir>`. Re-runnable. Offline, so its LLM timeout is 120 s — the 15 s bound in §9 is for the live engine only.
